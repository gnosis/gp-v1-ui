...
const OnchainOrderbookPriceEstimation: React.FC<OnchainOrderbookPriceEstimationProps> = (props) => {
  ...
  if (priceEstimation) {
    ...
    invertedPrice = formatPriceToPrecision(isPriceInverted ? priceEstimation : invertPrice(priceEstimation))
  }
  const displayPrice =
    price === 'Infinity' || invertedPrice === 'Infinity' ? 'N/A' : !wasPriorityAdjusted ? invertedPrice : price
